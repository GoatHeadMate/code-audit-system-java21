...
## 7. 阶段 3 实测（2980 文件 / 22139 方法真实样本）

| 指标 | 关闭 | 启用 | 变化 |
|---|---|---|---|
| 分析总耗时 | 10.9s | 24.8s | **约 2.3×** |
| unresolved 调用 | 37512 | 36106 | −3.7% |
| 发现 sink | 360 | 379 | +5.3% |
| 方法数 | 22139 | 22139 | 持平 |

结论：无 classpath 下 SymbolSolver 收益有限（unresolved −3.7%、sink +5.3%），而耗时翻倍有余。
**开关默认保持关闭**；仅在可提供依赖 jar / classpath、或对内部调用精度要求高的场景启用：
`-Daudit.symbol-solver=true` 或环境变量 `SYMBOL_SOLVER_ENABLED=true`。
